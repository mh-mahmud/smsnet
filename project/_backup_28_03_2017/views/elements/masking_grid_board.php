...
<div class="table-responsive">
	<table class="table table-striped table-bordered table-hover">
		...
		<?php if(count($records)>0){foreach($records as  $key=>$row):?>
		<tr class="<?php echo cycle(array('bg1','bg'));?>">
		   ...
		   <?php elseif($field=='users'): ?>
			<td class='lnk'><?php 
				...
				if(count($records[$key]['all_users'])>0){
					foreach($records[$key]['all_users'] as $val){
						
						echo $val['username'].'('.substr($val['title'], 0, 1).'), ';
					}	
				}else{
