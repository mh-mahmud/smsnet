	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tr class="<?php echo cycle(array('bg1','bg'));?>">
		   ...
		   <?php elseif($field=='link'): ?>
			...
			<?php if($gatype === 'edit'):?> 
			<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'].'/'.$row['type'] ?>" class="ajax_link" title="Click here for edit this item"><span class="action_btn btn-primary"><i class="fa fa-pencil"></i></span></a>
			...
			<?php elseif($gatype === 'approve'):?>
			<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'].'/'.$row['type'] ?>" class="ajax_link" title="Click here for view details"><span class="action_btn btn-success"><i class="fa fa-check"></i></span></a>
			
			<?php elseif($gatype === 'invoice'):?>
			<a href="#" title="<?php echo $row['id']; ?>" class="salesinvoice"><span class="action_btn btn-success"><i class="fa fa-money"></i></span></a>
		
			<?php elseif($gatype === 'approved'):?>
			<a href="#" title="<?php echo $row['id']; ?>" class="salesinvoice"><span class="action_btn btn-success"><i class="fa fa-money"></i></span></a>
		...
		<?php if(count($records)<=0 AND empty($records)):?>
		<tr>
		...
		<div class="col-xs-6">
			<div id="example2_info" class="dataTables_info">
				...
				<select name="rec_per_page" id='rec_per_page'>
					<?php echo html_options(array(2=>2,10=>10,30=>30,50=>50,100=>100,200=>200),$rec_per_page); ?>
				</select>		
...
$(document).ready(function(){
	$(".salesinvoice").click(function(){
		var id = $(this).attr('title');
			   $.ajax({
                type: "POST",
                url: "<?php echo site_url(); ?>/inventory/get_invoice",
                data: "id=" + id,
