	...
	<table class="table table-striped table-bordered table-hover">
		<thead>
			<tr>
				...
				<th width="180" align="center"><a href="#">Action</a></th>
				<?php endif?>	
			</tr>			
		</thead>						
		<tbody>
		...
		<tr class="<?php echo cycle(array('bg1','bg'));?>">
			...
			}else if(strtolower($row[$field])=='approved'){
				$status_title = '<span class="label label-success">'.ucfirst(strtolower($row[$field])).'</span>';
			}
			else{
			...
			<?php if(strtolower($row[$field])=='approved'){?>
				<?php echo $status_title; ?>
			<?php }else{?>
				<a href="<?php echo $site_url.$active_controller;?>/set_deposite_status/<?php echo $row['id']?>" class="<?=strtolower($row[$field])?>"><?php echo $status_title; ?> </a>
			<?php }?>
			</td>	 
			<?php 
		   ...
		   <?php elseif($field=='link'): ?>
			...
			<?php if($gatype === 'edit' || $gatype === 'walletEdit'):?> 
			
			<?php if($row[$field]=='Approved'){?>
				<span class="label label-success">Action Completed</span>
			<?php }else{?>
			...
			<?php elseif($gatype === 'credit' || $gatype ==='credit' ):?>
			<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'] ?>" title="Click here for credit"><span class="action_btn btn-success"><i class="fa fa-credit-card"></i></span></a>
			
			<?php elseif($gatype === 'sms' || $gatype ==='sms' ):?>
			<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'] ?>" title="Click here for sms"><span class="action_btn btn-success"><i class="fa fa-comment"></i></span></a>
			
			<?php elseif($gatype === 'login' || $gatype ==='login' ):?>
			<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'] ?>" title="Click here for login" ><span class="action_btn btn-primary"><i class="fa fa-sign-in"></i></span></a>
		...
		<div class="col-xs-6">
			<div id="example2_info" class="dataTables_info">
				...
				<select name="rec_per_page" id='rec_per_page'>
					<?php echo html_options(array(20=>20,30=>30,50=>50,100=>100,200=>200),$rec_per_page); ?>
				</select>		
...
$(document).ready(function() {
	...
	if(status_type==1)
	{
		var menuItems=[
			...
			{title:'<i class="fa fa-times-circle text-danger status"> Pending</i>',value:'Pending'}				  
		];
	}else{
		var menuItems=[
	...
	$(".change_status_menu").click(function() {
		location.reload();
	});
