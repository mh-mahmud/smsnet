...
class Report_center extends Bindu_Controller {
    ...
    function outboxsmsresend($id) {
        ...
        // delete from sms_outbox list
        $this->db->query("DELETE FROM sms_outbox WHERE id='{$failmessages['id']}'");

        // get mask id from mask name
        if($failmessages['mask']) {
            $failmessages['mask'] = $this->db->query("SELECT id FROM sms_senderid WHERE senderID='{$failmessages['mask']}'")->row()->id;
        }
        else {
            $failmessages['mask'] = "";
        }

        // send back to the sms_sentmessages
        $data['user_id'] = $failmessages['user_id'];
        $data['message'] = $failmessages['message'];
        $data['senderID'] = $failmessages['mask'];
        $data['recipient'] = $failmessages['destmn'];

        $data['date'] = date('Y-m-d h:i:s');
        $data['sentFrom'] = '8167';
        $data['is_unicode'] = $failmessages['is_unicode'];
        $data['IP'] = $this->get_client_ip();
        $data['sms_type'] = "SentMessage";
