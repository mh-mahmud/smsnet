        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($rate_id) {
                foreach ($operator_options as $key => $opval) {
                    ...
                    $datas['buying_rate'] = $this->input->post('selling_rate' . $opval['id']);
                    $datas['selling_rate'] = 0; 
    ...
    function edit_rate($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($rate_id) {
                foreach ($operator_options as $key => $opval) {
                    $datas['rate_id'] = $id;
                    $datas['operator_id'] = $this->input->post('operator_id' . $key);$datas['buying_rate'] = $this->input->post('selling_rate' . $key); //$this->operatormodel->get_buying_rate_operatorid($opval['id']);
                    $datas['selling_rate'] = 0;
                    $datas['created_by'] = $this->user_id;
                    $datas['created'] = $this->current_date();
                    $this->operatormodel->addDefaultPrice($datas);
