	...
	<table class="table table-striped table-bordered table-hover">
		<thead>
			<tr>
				...
				<th width="130" align="center"><a href="#">Action</a></th>
				<?php endif?>	
			</tr>			
		</thead>						
		<tbody>
		...
		<tr class="<?php echo cycle(array('bg1','bg'));?>">
		   ...
		   <?php elseif($field=='link'): ?>
			...
			<?php elseif(in_array($field,array('message'))):?>
			...
			<?php elseif($gatype === 'resend' || $gatype ==='resend' ):?>
				<?php if($row['status']!='Sent'){?>
				<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'] ?>" title="Click here for resend message" ><span class="action_btn btn-success"><i class="fa fa-envelope"></i></span></a>
				<?php }?>
			<?php elseif($gatype === 'outboxsmsresend' || $gatype ==='outboxsmsresend' ):?>
