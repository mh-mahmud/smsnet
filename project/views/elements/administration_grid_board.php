...
<div class="table-responsive">
	<table class="table table-striped table-bordered table-hover">
		...
		<tr class="<?php echo cycle(array('bg1','bg'));?>">
			...
			<?php }?>
			<?php if($this->router->fetch_method()=='operatorindex'){?>
			<a href="<?php echo $site_url.$active_controller;?>/operator_set_status/<?php echo $row['id']?>" class="<?=strtolower($row[$field])?>"><?php echo $status_title; ?> </a>
			<?php }?>
			<?php if($this->router->fetch_method()=='channelindex'){?>
			<a href="<?php echo $site_url.$active_controller;?>/channel_set_status/<?php echo $row['id']?>" class="<?=strtolower($row[$field])?>"><?php echo $status_title; ?> </a>
			<?php }?>
			<?php if($this->router->fetch_method()=='routeindex'){?>
			<a href="<?php echo $site_url.$active_controller;?>/route_set_status/<?php echo $row['id']?>" class="<?=strtolower($row[$field])?>"><?php echo $status_title; ?> </a>
			<?php }?>
			</td>	 
			<?php 
			...
			<?php elseif($field=='has_mask'): ?>
				<?php if($row[$field]==1){?>
					<td><?php echo 'YES'; ?></td>
				<?php }else if($row[$field]==0){?>
					<td><?php echo 'NO'; ?></td>
				<?php }else{?>
				<td><?php echo 'ALL'; ?></td>
				<?php }?>
		...
		<div class="col-xs-6">
			<div id="example2_info" class="dataTables_info">
				...
				<select name="rec_per_page" id='rec_per_page'>
					<?php echo html_options(array(10=>10,20=>20,30=>30,50=>50,100=>100,200=>200),$rec_per_page); ?>
				</select>		
